...
from backend.lbfl.src.model.tree import find_node, calculate_experts_features
from backend.lbfl.src.model.tree import tree_from_ngram, Tree, get_stats, standardize
from backend.lbfl.src.model.tree import build_tree, update_tree
...
class VotingExperts:
    ...
    def count_ngrams(self, file_name):
        ...
        logger.debug(
            f"{srilm_path}/ngram-count -order {self.tree_depth} -text {self.split_file_name} -no-sos -no-eos -write {out_file} ")
        start_time = time.time()
        subprocess.call(
            f"{srilm_path}/ngram-count -order {self.tree_depth} -text {self.split_file_name} -no-sos -no-eos -write {out_file}",
            shell=True)
        logger.debug(
            f"calculating ngram of order {self.tree_depth} for file {self.split_file_name} took ----- {(time.time() - start_time)} seconds -----")
